...
public class MapScript : MonoBehaviour
{
    ...
    private static void texturize(double perlinHeight, int buildingType, GameObject obj, float spriteSize)
    {
        ...
        {
            spriteRenderer.sprite = Sprite.Create(GlobalVariableHandler.roadTexture, new(0, 0, GlobalVariableHandler.roadTexture.width, GlobalVariableHandler.roadTexture.height), new(0.0f, 0.0f));
        }
        else
        {
            ...
            spriteRenderer.sprite = Sprite.Create(textures[(height - GlobalVariableHandler.waterline) / singleInterval + 1], new(0, 0, textures[(height - GlobalVariableHandler.waterline) / singleInterval + 1].width, textures[(height - GlobalVariableHandler.waterline) / singleInterval + 1].height), new(0.0f, 0.0f));
        }
    }
            ...
            for (int j = 0; j < sizeX; ++j) 
            {
                switch(buldings[i,j])
                {
                    ...
                    case (int)Constants.Buildings.Base:
                        ...
                        prop.setName(GlobalVariableHandler.players[baseIndex++].Name);
                        break;
